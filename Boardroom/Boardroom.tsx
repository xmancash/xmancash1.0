...
const Boardroom: React.FC = () => {
  ...
  const scalingFactor = useMemo(
    () => (cashStat ? Number(cashStat.priceInDAI).toFixed(6) : null),
    [cashStat],
  );
  ...
  const nextEpoch = useMemo(() => moment(prevEpoch).add(12, 'h').toDate(), [prevEpoch]);

  const { prevWithdrawAllocation, nextWithdrawAllocation, period } = useBoardroomAllocationTimes();

  const prevEpoch1 = useMemo(
    () =>
  ...
  const nextEpoch1 = useMemo(() => moment(prevEpoch1).add((period * 3), 's').toDate(), [prevEpoch1]);
      ...
      return (
        <StyledNoticeWrapper>
          <Notice color="green">
            ...
            <b>to continue earning XMC seigniorage.</b>
          </Notice>
        </StyledNoticeWrapper>
      );
    }
      ...
      <Switch>
        <Page>
          <PageHeader
            ...
            subtitle="Deposit XMAN Shares and earn inflationary rewards"
          />
          <LaunchCountdown
            ...
            <StyledBoardroom>
              ...
              {!usingOldBoardroom && (
                ...
                <>
                  <div>
                    <Button
                      disabled={stakedBalance.eq(0) || isDisabled}
                      onClick={onRedeem}
